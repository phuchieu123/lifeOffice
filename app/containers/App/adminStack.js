...
export function AdminStackScreen(props) {
  const { hrmRole, bosRole, filemanagerRole, taskRole, HrmTimekeepingTableRole, hasDriver, clientId } = props

  const [notifications, setNotifications] = useState({})
  const { isNotRead = 0 } = notifications

  useEffect(() => {
    const onNoti = DeviceEventEmitter.addListener("notification", setNotifications)
    return () => {
      onNoti.remove()
    }
  }, [])
  ...
  return (
    <AdminTab.Navigator initialRouteName="DashBoard" barStyle={{ backgroundColor: theme.brandPrimary }}>
      <AdminTab.Screen
        options={{
          tabBarLabel: 'Trang chủ',
          tabBarIcon: ({ color }) => (
            <Icon name="home" type="FontAwesome" style={{ color: color, fontSize: 30, marginTop: -5 }} />
          ),
        }}
        name="DashBoard"
      ...
      {!hrmRole.GET || !HrmTimekeepingTableRole.GET ? null :
        <AdminTab.Screen
          options={{
            tabBarLabel: "HRM",
            tabBarIcon: ({ color }) => <Icon name="users" type="FontAwesome" style={{ color: color, fontSize: 20 }} />,
          }}
          name="Hrm"
          component={HrmPage}
        />
